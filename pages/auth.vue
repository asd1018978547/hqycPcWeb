<template>
  <div class="main">
    <v-top></v-top>
    <div class="topCon">
      <div class="container flex flex-c">
        <span>
          <v-logo></v-logo>
        </span>
        <span
          class="flex1"
          style="font-size: 24px;color: #656565;margin-left: 20px;padding: 5px 20px;border-left: 2px solid #ccc;"
        >会员认证</span>
      </div>
    </div>
    <div
      class="tableCon marginTop20"
      v-if="(status == -2 || status == 0 || status == 1 || status == 2) && selectedIndex == -1"
    >
      <div class="container white-bg" style="padding:20px;">
        <p style="padding-bottom:20px;color:#666">请选择您的认证类型：</p>
        <el-row :gutter="12">
          <el-col
            :span="8"
            v-if="((status == 2)|| ((status == 1 || status == 0) && autht == 1)) || status == -2"
          >
            <el-card class="auth_card" :class="getClass(1)" shadow="hover">
              微商代购
              <span
                v-if="status == -2 || ((status == 1 || status == 2) && autht != 1)"
                style="float:right;color:#409EFF"
                @click="selected(1)"
              >去认证</span>
              <span v-else style="float:right;color:#409EFF" @click="selected(1)">查看详情</span>
            </el-card>
          </el-col>
          <el-col
            :span="8"
            v-if="((status == 2)|| ((status == 1 || status == 0) && autht == 2)) || status == -2"
          >
            <el-card class="auth_card" :class="getClass(2)" shadow="hover">
              网络店铺
              <span
                v-if="status == -2 || ((status == 1 || status == 2) && autht != 2)"
                style="float:right;color:#409EFF"
                @click="selected(2)"
              >去认证</span>
              <span v-else style="float:right;color:#409EFF" @click="selected(2)">查看详情</span>
            </el-card>
          </el-col>
          <el-col
            :span="8"
            v-if="((status == 2)||((status == 1 || status == 0) && autht == 3)) || status == -2"
          >
            <el-card class="auth_card" :class="getClass(3)" shadow="hover">
              实体店铺
              <span
                v-if="status == -2 || ((status == 1 || status == 2) && autht != 3)"
                style="float:right;color:#409EFF"
                @click="selected(3)"
              >去认证</span>
              <span v-else style="float:right;color:#409EFF" @click="selected(3)">查看详情</span>
            </el-card>
          </el-col>
          <el-col
            :span="8"
            v-if="((status == 2)||((status == 1 || status == 0) && autht == 4)) || status == -2"
          >
            <el-card class="auth_card" :class="getClass(4)" shadow="hover">
              自营电商平台
              <span
                v-if="status == -2 || ((status == 1 || status == 2) && autht != 4)"
                style="float:right;color:#409EFF"
                @click="selected(4)"
              >去认证</span>
              <span v-else style="float:right;color:#409EFF" @click="selected(4)">查看详情</span>
            </el-card>
          </el-col>
          <el-col
            :span="8"
            v-if="((status == 2)||((status == 1 || status == 0) && autht == 6)) || status == -2"
          >
            <el-card class="auth_card" :class="getClass(6)" shadow="hover">
              个人
              <span
                v-if="status == -2 || ((status == 1 || status == 2) && autht != 6)"
                style="float:right;color:#409EFF"
                @click="selected(6)"
              >去认证</span>
              <span v-else style="float:right;color:#409EFF" @click="selected(6)">查看详情</span>
            </el-card>
          </el-col>
        </el-row>
      </div>
    </div>
    <div class="tableCon marginTop20" v-if="selectedIndex == 1">
      <div class="container white-bg">
        <div class="table">
          <div class="input_con" v-if="FailReason != null">
            <span class="input_label" style="color:#f55">认证未通过：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              <i style="color:#f55">{{ FailReason }}</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">认证类型：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              {{ authType }}
              <i style="color:#f55">（注：审核中或审核通过后，将不可更改相关信息）</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>经营品类：
            </span>
            <div class="input_link link_800_28">
              <el-checkbox-group
                v-model="s_1.operationCategory"
                :disabled="status == 0 || status == 1"
              >
                <el-checkbox :disabled="status == 0 || status == 1" label="002,母婴用品">母婴用品</el-checkbox>
                <el-checkbox :disabled="status == 0 || status == 1" label="003,食品保健">食品保健</el-checkbox>
                <el-checkbox :disabled="status == 0 || status == 1" label="004,生活用品">生活用品</el-checkbox>
                <el-checkbox :disabled="status == 0 || status == 1" label="005,美妆个护">美妆个护</el-checkbox>
                <el-checkbox :disabled="status == 0 || status == 1" label="006,儿童玩具">儿童玩具</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">月销售额(万)：</span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_1.sales"
                size="mini"
                placeholder="请输入月销售额"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>微信号：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_1.weChat"
                size="mini"
                placeholder="请输入微信号"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>手机号：
            </span>
            <div class="input_link w500">
              <el-select
                size="mini"
                v-model="s_1.phoneMode"
                slot="prepend"
                class="input_selete"
                :disabled="status == 0 || status == 1"
                placeholder="请选择"
              >
                <el-option label="中国大陆 +86" value="1"></el-option>
                <el-option label="中国台湾 +886" value="2"></el-option>
                <el-option label="中国香港 +82" value="3"></el-option>
                <el-option label="澳大利亚 +61" value="4"></el-option>
              </el-select>
              <el-input
                class="w240"
                :disabled="status == 0 || status == 1"
                v-model="s_1.phone"
                size="mini"
                placeholder="请输入手机号"
              ></el-input>
              <el-button
                v-if="status != 0 && status != 1"
                class="mrl10"
                type="primary"
                size="mini"
                @click="getPhoneCode(1)"
                :disabled="status == 0 || status == 1"
              >获取验证码</el-button>
            </div>
          </div>
          <div class="input_con" v-if="status != 0 && status != 1">
            <span class="input_label">
              <i class="color-red">*</i>验证码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_1.Code"
                size="mini"
                placeholder="请输入验证码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>姓名：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_1.userName"
                size="mini"
                placeholder="请输入姓名"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证号码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_1.idCardNo"
                size="mini"
                placeholder="请输入身份证号码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证或护照正面照片：
            </span>
            <div class="flex">
              <el-upload
                :disabled="status == 0 || status == 1"
                class="avatar-uploader"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardFrontImg')"
              >
                <div class="image_con">
                  <img v-if="s_1.idCardFrontImg" :src="baseUrl+s_1.idCardFrontImg" class="avatar" />
                  <p v-if="!s_1.idCardFrontImg">点击上传图片</p>
                  <span v-if="s_1.idCardFrontImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span
                class="color99 marginLeft10"
                v-show="!s_1.idCardFrontImg"
              >(上传的身份证或护照正面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证或护照反面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :disabled="status == 0 || status == 1"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardBehindImg')"
              >
                <div class="image_con">
                  <img v-if="s_1.idCardBehindImg" :src="baseUrl+s_1.idCardBehindImg" class="avatar" />
                  <p v-if="!s_1.idCardBehindImg">点击上传图片</p>
                  <span
                    v-if="s_1.idCardBehindImg && status != 0 && status != 1"
                    class="reUpload"
                  >重新上传</span>
                </div>
              </el-upload>
              <span
                class="color99 marginLeft10"
                v-show="!s_1.idCardBehindImg"
              >(上传的身份证或护照反面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>地址：
            </span>
            <div class="input_address">
              <el-radio v-model="s_1.country" label="中国" :disabled="status == 0 || status == 1">中国</el-radio>
              <el-radio v-model="s_1.country" label="国外" :disabled="status == 0 || status == 1">国外</el-radio>
              <el-cascader
                v-show="s_1.country == '中国'"
                size="mini"
                style="width:100%;margin-top:10px;"
                v-model="s_1.area"
                separator="-"
                :disabled="status == 0 || status == 1"
                placeholder="请选择地址"
                :props="areaOptions"
                :options="areaData2"
                filterable
              ></el-cascader>
            </div>
          </div>
          <div class="btn_group" v-if="status != 0 && status != 1">
            <span class="btn_sub" @click="subMite">提交</span>
          </div>
        </div>
      </div>
    </div>
    <div class="tableCon marginTop20" v-if="selectedIndex == 2">
      <div class="container white-bg">
        <div class="table">
          <div class="input_con" v-if="FailReason != null">
            <span class="input_label" style="color:#f55">认证未通过：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              <i style="color:#f55">{{ FailReason }}</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">认证类型：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              {{ authType }}
              <i style="color:#f55">（注：审核通过后，将不可更改相关信息）</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>销售平台：
            </span>
            <div class="input_link link_800_28">
              <el-select
                :disabled="status == 0 || status == 1"
                v-model="s_2.platform"
                size="mini"
                placeholder="请选择销售平台"
              >
                <el-option v-for="(item, i) in plf" :key="i" :label="item.name" :value="item.value"></el-option>
              </el-select>
              <el-popover placement="top" width="180" v-model="visible2">
                <el-input
                  style="margin-bottom:10px;"
                  v-model="addplfname"
                  size="mini"
                  placeholder="请输入销售平台名称"
                ></el-input>
                <div style="text-align: center; margin: 0">
                  <el-button size="mini" type="text" @click="cancelPLF">取消</el-button>
                  <el-button type="primary" size="mini" @click="addPLF">确定</el-button>
                </div>
                <el-button
                  v-if="status != 0 && status != 1"
                  slot="reference"
                  type="success"
                  size="mini"
                >添加平台</el-button>
              </el-popover>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店铺名称：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.shopName"
                size="mini"
                placeholder="请输入店铺名称"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店铺网址：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.webLink"
                size="mini"
                placeholder="http或者https开头"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>经营类目：
            </span>
            <div class="input_link link_800_28">
              <el-checkbox-group
                :disabled="status == 0 || status == 1"
                v-model="s_2.operationCategory"
              >
                <el-checkbox label="002,母婴用品">母婴用品</el-checkbox>
                <el-checkbox label="003,食品保健">食品保健</el-checkbox>
                <el-checkbox label="004,生活用品">生活用品</el-checkbox>
                <el-checkbox label="005,美妆个护">美妆个护</el-checkbox>
                <el-checkbox label="006,儿童玩具">儿童玩具</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">月销售额(万)：</span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.sales"
                size="mini"
                placeholder="请输入月销售额"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>联系电话：
            </span>
            <div class="input_link w500">
              <el-select
                size="mini"
                v-model="s_2.phoneMode"
                slot="prepend"
                class="input_selete"
                :disabled="status == 0 || status == 1"
                placeholder="请选择"
              >
                <el-option label="中国大陆 +86" value="1"></el-option>
                <el-option label="中国台湾 +886" value="2"></el-option>
                <el-option label="中国香港 +82" value="3"></el-option>
                <el-option label="澳大利亚 +61" value="4"></el-option>
              </el-select>
              <el-input
                class="w240"
                :disabled="status == 0 || status == 1"
                v-model="s_2.phone"
                size="mini"
                placeholder="请输入联系电话"
              ></el-input>
              <el-button
                v-if="status != 0 && status != 1"
                class="mrl10"
                type="primary"
                size="mini"
                @click="getPhoneCode(2)"
                :disabled="status == 0 || status == 1"
              >获取验证码</el-button>
            </div>
          </div>
          <div class="input_con" v-if="status != 0 && status != 1">
            <span class="input_label">
              <i class="color-red">*</i>验证码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.Code"
                size="mini"
                placeholder="请输入验证码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店主姓名：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.userName"
                size="mini"
                placeholder="请输入店主姓名"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证号码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_2.idCardNo"
                size="mini"
                placeholder="请输入身份证号码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证正面照片：
            </span>
            <div class="flex">
              <el-upload
                :disabled="status == 0 || status == 1"
                class="avatar-uploader"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardFrontImg')"
              >
                <div class="image_con">
                  <img v-if="s_2.idCardFrontImg" :src="baseUrl+s_2.idCardFrontImg" class="avatar" />
                  <p v-if="!s_2.idCardFrontImg">点击上传图片</p>
                  <span v-if="s_2.idCardFrontImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_2.idCardFrontImg">(上传的身份证正面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证反面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardBehindImg')"
              >
                <div class="image_con">
                  <img v-if="s_2.idCardBehindImg" :src="baseUrl+s_2.idCardBehindImg" class="avatar" />
                  <p v-if="!s_2.idCardBehindImg">点击上传图片</p>
                  <span
                    v-if="s_2.idCardBehindImg&& status != 0 && status != 1"
                    class="reUpload"
                  >重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_2.idCardBehindImg">(上传的身份证反面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>营业执照照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :disabled="status == 0 || status == 1"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('licensImg')"
              >
                <div class="image_con">
                  <img v-if="s_2.licensImg" :src="baseUrl+s_2.licensImg" class="avatar" />
                  <p v-if="!s_2.licensImg">点击上传图片</p>
                  <span v-if="s_2.licensImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_2.licensImg">(上传的营业执照照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店铺首页照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('shopImg')"
              >
                <div class="image_con">
                  <img v-if="s_2.shopImg" :src="baseUrl+s_2.shopImg" class="avatar" />
                  <p v-if="!s_2.shopImg">点击上传图片</p>
                  <span v-if="s_2.shopImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_2.shopImg">(上传的店铺首页照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>卖家中心照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('saleCenterImg')"
              >
                <div class="image_con">
                  <img v-if="s_2.saleCenterImg" :src="baseUrl+s_2.saleCenterImg" class="avatar" />
                  <p v-if="!s_2.saleCenterImg">点击上传图片</p>
                  <span v-if="s_2.saleCenterImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_2.saleCenterImg">(上传的卖家中心照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="btn_group" v-if="status != 0 && status != 1">
            <span class="btn_sub" @click="subMite">提交</span>
          </div>
        </div>
      </div>
    </div>
    <div class="tableCon marginTop20" v-if="selectedIndex == 3">
      <div class="container white-bg">
        <div class="table">
          <div class="input_con" v-if="FailReason != null">
            <span class="input_label" style="color:#f55">认证未通过：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              <i style="color:#f55">{{ FailReason }}</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">认证类型：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              {{ authType }}
              <i style="color:#f55">（注：审核通过后，将不可更改相关信息）</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店铺名称：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.shopName"
                size="mini"
                placeholder="请输入店铺名称"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>联系电话：
            </span>
            <div class="input_link w500">
              <el-select
                size="mini"
                v-model="s_3.phoneMode"
                slot="prepend"
                class="input_selete"
                :disabled="status == 0 || status == 1"
                placeholder="请选择"
              >
                <el-option label="中国大陆 +86" value="1"></el-option>
                <el-option label="中国台湾 +886" value="2"></el-option>
                <el-option label="中国香港 +82" value="3"></el-option>
                <el-option label="澳大利亚 +61" value="4"></el-option>
              </el-select>
              <el-input
                class="w240"
                :disabled="status == 0 || status == 1"
                v-model="s_3.phone"
                size="mini"
                placeholder="请输入联系电话"
              ></el-input>
              <el-button
                v-if="status != 0 && status != 1"
                class="mrl10"
                type="primary"
                size="mini"
                @click="getPhoneCode(3)"
                :disabled="status == 0 || status == 1"
              >获取验证码</el-button>
            </div>
          </div>
          <div class="input_con" v-if="status != 0 && status != 1">
            <span class="input_label">
              <i class="color-red">*</i>验证码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.Code"
                size="mini"
                placeholder="请输入验证码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>所在地：
            </span>
            <div class="input_address">
              <el-cascader
                size="mini"
                :disabled="status == 0 || status == 1"
                style="width:100%"
                v-model="s_3.area"
                separator="-"
                placeholder="请选择地址"
                :props="areaOptions"
                :options="areaData"
                filterable
              ></el-cascader>
              <el-input
                size="mini"
                type="textarea"
                :disabled="status == 0 || status == 1"
                class="descAddress"
                placeholder="请填写详细地址"
                v-model="s_3.address"
                :autosize="{ minRows: 4}"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>分店数量：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.shopCount"
                size="mini"
                placeholder="请输入分店数量"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>经营品类：
            </span>
            <div class="input_link link_800_28">
              <el-checkbox-group
                :disabled="status == 0 || status == 1"
                v-model="s_3.operationCategory"
              >
                <el-checkbox label="002,母婴用品">母婴用品</el-checkbox>
                <el-checkbox label="003,食品保健">食品保健</el-checkbox>
                <el-checkbox label="004,生活用品">生活用品</el-checkbox>
                <el-checkbox label="005,美妆个护">美妆个护</el-checkbox>
                <el-checkbox label="006,儿童玩具">儿童玩具</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">月销售额(万)：</span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.sales"
                size="mini"
                placeholder="请输入月销售额"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证号码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.idCardNo"
                size="mini"
                placeholder="请输入身份证号码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>是否拥有网络店铺：
            </span>
            <div class="input_link" style="line-height:28px">
              <el-radio
                :disabled="status == 0 || status == 1"
                v-model="s_3.hasNetWorkShop"
                :label="true"
              >是</el-radio>
              <el-radio
                :disabled="status == 0 || status == 1"
                v-model="s_3.hasNetWorkShop"
                :label="false"
              >否</el-radio>
            </div>
          </div>
          <div class="input_con" v-show="s_3.hasNetWorkShop">
            <span class="input_label">
              <i class="color-red">*</i>销售平台：
            </span>
            <div class="input_link link_800_28">
              <el-select
                :disabled="status == 0 || status == 1"
                v-model="s_3.platform"
                size="mini"
                placeholder="请选择销售平台"
              >
                <el-option v-for="(item, i) in plf" :key="i" :label="item.name" :value="item.value"></el-option>
              </el-select>
              <el-popover placement="top" width="180" v-model="visible2">
                <el-input
                  style="margin-bottom:10px;"
                  v-model="addplfname"
                  size="mini"
                  placeholder="请输入销售平台名称"
                ></el-input>
                <div style="text-align: center; margin: 0">
                  <el-button size="mini" type="text" @click="cancelPLF">取消</el-button>
                  <el-button type="primary" size="mini" @click="addPLF">确定</el-button>
                </div>
                <el-button
                  v-if="status != 0 && status != 1"
                  slot="reference"
                  type="success"
                  size="mini"
                >添加平台</el-button>
              </el-popover>
            </div>
          </div>
          <div class="input_con" v-show="s_3.hasNetWorkShop">
            <span class="input_label">
              <i class="color-red">*</i>店铺网址：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.webLink"
                size="mini"
                placeholder="http或者https开头"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>企业名称：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_3.companyName"
                size="mini"
                placeholder="请输入企业名称"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>店铺照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :show-file-list="false"
                :disabled="status == 0 || status == 1"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('shopImg')"
              >
                <div class="image_con">
                  <img v-if="s_3.shopImg" :src="baseUrl+s_3.shopImg" class="avatar" />
                  <p v-if="!s_3.shopImg">点击上传图片</p>
                  <span v-if="s_3.shopImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_3.shopImg">(上传的店铺照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>法人身份证正面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardFrontImg')"
              >
                <div class="image_con">
                  <img v-if="s_3.idCardFrontImg" :src="baseUrl+s_3.idCardFrontImg" class="avatar" />
                  <p v-if="!s_3.idCardFrontImg">点击上传图片</p>
                  <span v-if="s_3.idCardFrontImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_3.idCardFrontImg">(上传的法人身份证正面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>法人身份证反面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardBehindImg')"
              >
                <div class="image_con">
                  <img v-if="s_3.idCardBehindImg" :src="baseUrl+s_3.idCardBehindImg" class="avatar" />
                  <p v-if="!s_3.idCardBehindImg">点击上传图片</p>
                  <span
                    v-if="s_3.idCardBehindImg&& status != 0 && status != 1"
                    class="reUpload"
                  >重新上传</span>
                </div>
              </el-upload>
              <span
                class="color99 marginLeft10"
                v-show="!s_3.idCardBehindImg"
              >(上传的法人身份证反面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>营业执照照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :disabled="status == 0 || status == 1"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('licensImg')"
              >
                <div class="image_con">
                  <img v-if="s_3.licensImg" :src="baseUrl+s_3.licensImg" class="avatar" />
                  <p v-if="!s_3.licensImg">点击上传图片</p>
                  <span v-if="s_3.licensImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_3.licensImg">(上传的营业执照照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="btn_group" v-if="status != 0 && status != 1">
            <span class="btn_sub" @click="subMite">提交</span>
          </div>
        </div>
      </div>
    </div>
    <div class="tableCon marginTop20" v-if="selectedIndex == 4">
      <div class="container white-bg">
        <div class="table">
          <div class="input_con" v-if="FailReason != null">
            <span class="input_label" style="color:#f55">认证未通过：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              <i style="color:#f55">{{ FailReason }}</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">认证类型：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              {{ authType }}
              <i style="color:#f55">（注：审核通过后，将不可更改相关信息）</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>平台名称：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.shopName"
                size="mini"
                placeholder="请输入平台名称"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>平台属性：
            </span>
            <div class="input_link">
              <el-radio-group
                :disabled="status == 0 || status == 1"
                style="margin-top:8px;"
                v-model="s_4.shopType"
              >
                <el-radio :label="1">B2B</el-radio>
                <el-radio :label="2">B2C</el-radio>
              </el-radio-group>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>联系电话：
            </span>
            <div class="input_link w500">
              <el-select
                size="mini"
                v-model="s_4.phoneMode"
                slot="prepend"
                class="input_selete"
                :disabled="status == 0 || status == 1"
                placeholder="请选择"
              >
                <el-option label="中国大陆 +86" value="1"></el-option>
                <el-option label="中国台湾 +886" value="2"></el-option>
                <el-option label="中国香港 +82" value="3"></el-option>
                <el-option label="澳大利亚 +61" value="4"></el-option>
              </el-select>
              <el-input
                class="w240"
                :disabled="status == 0 || status == 1"
                v-model="s_4.phone"
                size="mini"
                placeholder="请输入联系电话"
              ></el-input>
              <el-button
                v-if="status != 0 && status != 1"
                class="mrl10"
                type="primary"
                size="mini"
                @click="getPhoneCode(4)"
                :disabled="status == 0 || status == 1"
              >获取验证码</el-button>
            </div>
          </div>
          <div class="input_con" v-if="status != 0 && status != 1">
            <span class="input_label">
              <i class="color-red">*</i>验证码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.Code"
                size="mini"
                placeholder="请输入验证码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>平台网址：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.webLink"
                size="mini"
                placeholder="请输入平台网址"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>经营品类：
            </span>
            <div class="input_link link_800_28">
              <el-checkbox-group
                :disabled="status == 0 || status == 1"
                v-model="s_4.operationCategory"
              >
                <el-checkbox label="002,母婴用品">母婴用品</el-checkbox>
                <el-checkbox label="003,食品保健">食品保健</el-checkbox>
                <el-checkbox label="004,生活用品">生活用品</el-checkbox>
                <el-checkbox label="005,美妆个护">美妆个护</el-checkbox>
                <el-checkbox label="006,儿童玩具">儿童玩具</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">月销售额(万)：</span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.sales"
                size="mini"
                placeholder="请输入月销售额"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">月订单数：</span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.orderCount"
                size="mini"
                placeholder="请输入月订单数"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>企业名称：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.companyName"
                size="mini"
                placeholder="请输入企业名称"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证号码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_4.idCardNo"
                size="mini"
                placeholder="请输入身份证号码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证正面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardFrontImg')"
              >
                <div class="image_con">
                  <img v-if="s_4.idCardFrontImg" :src="baseUrl+s_4.idCardFrontImg" class="avatar" />
                  <p v-if="!s_4.idCardFrontImg">点击上传图片</p>
                  <span v-if="s_4.idCardFrontImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_4.idCardFrontImg">(上传的身份证正面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证反面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :disabled="status == 0 || status == 1"
                :action="uploadUrl"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardBehindImg')"
              >
                <div class="image_con">
                  <img v-if="s_4.idCardBehindImg" :src="baseUrl+s_4.idCardBehindImg" class="avatar" />
                  <p v-if="!s_4.idCardBehindImg">点击上传图片</p>
                  <span
                    v-if="s_4.idCardBehindImg&& status != 0 && status != 1"
                    class="reUpload"
                  >重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_4.idCardBehindImg">(上传人身份证反面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>营业执照照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :disabled="status == 0 || status == 1"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('licensImg')"
              >
                <div class="image_con">
                  <img v-if="s_4.licensImg" :src="baseUrl+s_4.licensImg" class="avatar" />
                  <p v-if="!s_4.licensImg">点击上传图片</p>
                  <span v-if="s_4.licensImg && status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_4.licensImg">(上传的营业执照照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="btn_group" v-if="status != 0 && status != 1">
            <span class="btn_sub" @click="subMite">提交</span>
          </div>
        </div>
      </div>
    </div>
    <div class="tableCon marginTop20" v-if="selectedIndex == 6">
      <div class="container white-bg">
        <div class="table">
          <div class="input_con" v-if="FailReason != null">
            <span class="input_label" style="color:#f55">认证未通过：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              <i style="color:#f55">{{ FailReason }}</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">认证类型：</span>
            <div class="input_link" style="width:800px !important;line-height:28px;height:28px;">
              {{ authType }}
              <i style="color:#f55">（注：审核通过后，将不可更改相关信息）</i>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>姓名：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_6.userName"
                size="mini"
                placeholder="请输入姓名"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>联系电话：
            </span>
            <div class="input_link w500">
              <el-select
                size="mini"
                v-model="s_6.phoneMode"
                slot="prepend"
                class="input_selete"
                :disabled="status == 0 || status == 1"
                placeholder="请选择"
              >
                <el-option label="中国大陆 +86" value="1"></el-option>
                <el-option label="中国台湾 +886" value="2"></el-option>
                <el-option label="中国香港 +82" value="3"></el-option>
                <el-option label="澳大利亚 +61" value="4"></el-option>
              </el-select>
              <el-input
                class="w240"
                :disabled="status == 0 || status == 1"
                v-model="s_6.phone"
                size="mini"
                placeholder="请输入联系电话"
              ></el-input>
              <el-button
                v-if="status != 0 && status != 1"
                class="mrl10"
                type="primary"
                size="mini"
                @click="getPhoneCode(6)"
                :disabled="status == 0 || status == 1"
              >获取验证码</el-button>
            </div>
          </div>
          <div class="input_con" v-if="status != 0 && status != 1">
            <span class="input_label">
              <i class="color-red">*</i>验证码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_6.Code"
                size="mini"
                placeholder="请输入验证码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>收货地址：
            </span>
            <div class="input_address">
              <el-cascader
                size="mini"
                :disabled="status == 0 || status == 1"
                style="width:100%"
                v-model="s_6.area"
                separator="-"
                placeholder="请选择地址"
                :props="areaOptions"
                :options="areaData"
                filterable
              ></el-cascader>
              <el-input
                size="mini"
                type="textarea"
                :disabled="status == 0 || status == 1"
                class="descAddress"
                placeholder="请填写详细地址"
                v-model="s_6.address"
                :autosize="{ minRows: 4}"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证号码：
            </span>
            <div class="input_link">
              <el-input
                :disabled="status == 0 || status == 1"
                v-model="s_6.idCardNo"
                size="mini"
                placeholder="请输入身份证号码"
              ></el-input>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证正面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :disabled="status == 0 || status == 1"
                :show-file-list="false"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardFrontImg')"
              >
                <div class="image_con">
                  <img v-if="s_6.idCardFrontImg" :src="baseUrl+s_6.idCardFrontImg" class="avatar" />
                  <p v-if="!s_6.idCardFrontImg">点击上传图片</p>
                  <span v-if="s_6.idCardFrontImg&& status != 0 && status != 1" class="reUpload">重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_6.idCardFrontImg">(上传的身份证正面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="input_con">
            <span class="input_label">
              <i class="color-red">*</i>身份证反面照片：
            </span>
            <div class="flex">
              <el-upload
                class="avatar-uploader"
                :action="uploadUrl"
                :show-file-list="false"
                :disabled="status == 0 || status == 1"
                :on-change="handleAvatarChange"
                :on-error="handleAvatarError"
                :on-success="handleAvatarSuccess"
                :on-progress="handleAvatarProgress"
                :before-upload="beforeAvatarUpload"
                @click.native="upIndex('idCardBehindImg')"
              >
                <div class="image_con">
                  <img v-if="s_6.idCardBehindImg" :src="baseUrl+s_6.idCardBehindImg" class="avatar" />
                  <p v-if="!s_6.idCardBehindImg">点击上传图片</p>
                  <span
                    v-if="s_6.idCardBehindImg&& status != 0 && status != 1"
                    class="reUpload"
                  >重新上传</span>
                </div>
              </el-upload>
              <span class="color99 marginLeft10" v-show="!s_6.idCardBehindImg">(上传的身份证反面照片最大不可超过2M)</span>
            </div>
          </div>
          <div class="btn_group" v-if="status != 0 && status != 1">
            <span class="btn_sub" @click="subMite">提交</span>
          </div>
        </div>
      </div>
    </div>
    <v-bottom></v-bottom>
  </div>
</template>
<script>
import moment from "moment";
import { ali_oss_data } from "~/_config/config.js";
let OSS = require("ali-oss");
let imgSavePath =
  ali_oss_data.URL_IMG_HQYC + "auth/" + moment().format("YYYYMM") + "/img/";
import vLogo from "~/components/vLogo";
import vTop from "~/components/vTop";
import vBottom from "~/components/vBottom";
import { API } from "~/_config/API.js";
import { RegExp, AREA } from "~/tools/index";
import { _CONFIG } from "~/_config/config.js";
export default {
  components: {
    vLogo,
    vBottom,
    vTop
  },
  data() {
    return {
      uploadUrl: "",
      addplfname: "",
      visible3: false,
      visible2: false,
      kaname: "",
      status: -2,
      plf: [
        { name: "淘宝", value: "淘宝" },
        { name: "天猫", value: "天猫" },
        { name: "京东", value: "京东" },
        { name: "1688", value: "1688" },
        { name: "拼多多", value: "拼多多" },
        { name: "有赞", value: "有赞" },
        { name: "苏宁易购", value: "苏宁易购" },
        { name: "贝贝", value: "贝贝" },
        { name: "微店", value: "微店" }
      ],
      sp: [
        { storePort: "广州口岸" },
        { storePort: "郑州口岸" },
        { storePort: "重庆口岸" },
        { storePort: "宁波口岸" }
      ],
      areaData: [],
      areaData2: [],
      baseUrl: ali_oss_data.URL_IMG_SHOW,
      loading: true,
      refuseReason: "",
      selectImgUrl: "",
      authType: "默认类型",
      autht: "",
      areaData: [],
      FailReason: null,
      areaOptions: {
        value: "Code",
        label: "Name",
        children: "Children"
      },
      s_1: {
        area: [],
        region: "",
        phone: "",
        sales: "",
        country: "",
        phoneMode: "1",
        weChat: "",
        Code: "",
        userName: "",
        smsCodeType: 4,
        address: "",
        idCardNo: "",
        idCardFrontImg: null,
        idCardBehindImg: null,
        operationCategory: [],
        interestCategory: []
      },
      s_2: {
        shopName: "",
        region: "",
        webLink: "",
        sales: "",
        phone: "",
        platform: "",
        phoneMode: "1",
        smsCodeType: 4,
        operationCategory: [],
        interestCategory: [],
        userName: "",
        Code: "",
        idCardNo: "",
        licensImg: null,
        idCardFrontImg: null,
        idCardBehindImg: null,
        saleCenterImg: null,
        shopImg: null
      },
      s_3: {
        shopImg: null,
        region: "",
        idCardFrontImg: null,
        webLink: "",
        smsCodeType: 4,
        phoneMode: "1",
        platform: "",
        idCardBehindImg: null,
        licensImg: null,
        Code: "",
        shopName: "",
        smsCodeType: 4,
        idCardNo: "",
        hasNetWorkShop: null,
        shopCount: "",
        phone: "",
        area: [],
        companyName: "",
        address: "",
        operationCategory: [],
        interestCategory: [],
        sales: ""
      },
      s_4: {
        phoneMode: "1",
        shopName: "",
        region: "",
        Code: "",
        webLink: "",
        shopType: "",
        smsCodeType: 4,
        orderCount: "",
        idCardNo: "",
        operationCategory: [],
        sales: "",
        interestCategory: [],
        companyName: "",
        phone: "",
        idCardFrontImg: null,
        idCardBehindImg: null,
        licensImg: null
      },
      myFile: "",
      s_6: {
        userName: "",
        Code: "",
        region: "",
        area: [],
        address: "",
        phoneMode: "1",
        smsCodeType: 4,
        phone: "",
        idCardNo: "",
        idCardFrontImg: null,
        idCardBehindImg: null
      },
      selectedIndex: -1,
      //城市选择器
      areaOptions: {
        value: "Code",
        label: "Name",
        children: "Children"
      }
    };
  },
  methods: {
    //上传到ali-OSS
    uploadImgToOSS(file, imgName) {
      let client = new OSS(ali_oss_data.PARAMS_OBJ);
      async function put() {
        try {
          let result = await client.put(imgName, file);
        } catch (e) {
          console.log(e);
        }
      }
      put();
    },
    getClass(v) {
      if (this.status == 0 && v == this.autht) {
        return "auth_card_shing";
      } else if (this.status == 1 && v == this.autht) {
        return "auth_card_active";
      } else if (this.status == 2 && v == this.autht) {
        return "auth_card_passed";
      } else {
        return "auth_card_default";
      }
    },
    getPhoneCode(val) {
      let flag = false;
      switch (this["s_" + val].phoneMode) {
        case "1":
          flag = RegExp.phone.test(this["s_" + val].phone);
          break;
        case "2":
          flag = RegExp.TWphone.test("886" + this["s_" + val].phone);
          break;
        case "3":
          flag = RegExp.HKphone.test("82" + this["s_" + val].phone);
          break;
        case "4":
          flag = RegExp.AUphone.test("61" + this["s_" + val].phone);
          break;
        default:
          flag = false;
      }
      let p = "";
      switch (this["s_" + val].phoneMode) {
        case "1":
          break;
        case "2":
          p = "886";
          break;
        case "3":
          p = "82";
          break;
        case "4":
          p = "61";
          break;
        default:
          return false;
      }
      if (flag) {
        API.getCode({
          phone: this["s_" + val].phone,
          smsCodeType: 4,
          region: p,
          templateId: this["s_" + val].phoneMode == 1 ? 9 : 10
        }).then(res => {
          this.$message.success(res.Message);
        });
      } else {
        this.$message.error("请输入正确的手机号");
      }
    },
    cancelKA() {
      this.kaname = "";
      this.visible3 = false;
    },
    addKA() {
      if (this.kaname == "") {
        this.$message.error("请输入口岸名称");
        return;
      }
      this.sp.push({
        storePort: this.kaname
      });
      this.visible3 = false;
      this.$nextTick(() => {
        this["s_" + this.selectedIndex].storePort.push(
          JSON.parse(JSON.stringify(this.kaname))
        );
        this.kaname = "";
      });
    },
    cancelPLF() {
      this.visible2 = false;
      this.addplfname = "";
    },
    addPLF() {
      if (this.addplfname == "") {
        this.$message.error("请输入平台名称");
        return;
      }
      this.plf.push({
        name: this.addplfname,
        value: this.addplfname
      });
      this.visible2 = false;
      this.$nextTick(() => {
        this["s_" + this.selectedIndex].platform = JSON.parse(
          JSON.stringify(this.addplfname)
        );
        this.addplfname = "";
      });
    },
    selected(index) {
      this.selectedIndex = index;
      switch (index) {
        case 1:
          this.authType = "微商代购";
          break;
        case 2:
          this.authType = "网络店铺";
          break;
        case 3:
          this.authType = "实体门店";
          break;
        case 4:
          this.authType = "网络平台";
          break;
        case 6:
          this.authType = "批采企业";
          break;
      }
    },
    handleAvatarError(err, file, fileList) {
      this.loadDiv.close();
    },
    handleAvatarProgress(event, file, fileList) {
      this.loadDiv = this.$loading({
        lock: true,
        text: "文件上传中...",
        spinner: "el-icon-loading",
        background: "rgba(0, 0, 0, 0.7)"
      });
    },
    upIndex(str) {
      this.selectImgUrl = str;
    },
    handleAvatarSuccess(res, file, fileList) {
      this.loadDiv.close();
      if (res.ResponseId == 0) {
        let name = Math.floor(Math.random() * 1000)+new Date().getTime();
        let imageType = this.myFile.name.substring(
          this.myFile.name.lastIndexOf(".") + 1
        );
        let imgPath = imgSavePath + name + "." + imageType;
        this.uploadImgToOSS(this.myFile, imgPath);
        setTimeout(() => {
          this["s_" + this.selectedIndex][this.selectImgUrl] = imgPath;
        }, 500);
      } else if (res.ResponseId == -1) {
        this.$store.commit("LOGOUT");
        this.$message({
          message: "登录过期，请重新登录",
          type: "error",
          duration: 1000,
          onClose: () => {
            this.$router.push({ path: "/login" });
          }
        });
      } else {
        this.$message({
          message: res.Message,
          type: "error"
        });
      }
    },
    beforeAvatarUpload(file) {
      const isLt2M = file.size / 1024 / 1024 < 2;
      if (!isLt2M) {
        this.$message.error("上传图片大小不能超过 2MB!");
      } else {
        this.myFile = file;
      }
      return isLt2M;
    },
    handleAvatarChange(file, fileList) {},
    subMite() {
      let p = JSON.parse(JSON.stringify(this["s_" + this.selectedIndex]));
      let c = this.checkPar(p, this.selectedIndex);
      if (c.flag) {
        this.$message.error(c.msg);
        return;
      } else {
        c.par.token = localStorage.getItem("user")
          ? JSON.parse(localStorage.getItem("user")).token
          : null;
        if (c.par.id) {
          c.par.retailerAuthId = c.par.id;
        }
        let region = "";
        switch (c.par.phoneMode) {
          case "1":
            break;
          case "2":
            region = "886";
            break;
          case "3":
            region = "82";
            break;
          case "4":
            region = "61";
            break;
          default:
            return false;
        }
        c.par.region = region;
        API.subAuth(c.par).then(res => {
          if (res.ResponseId == 0) {
            this.$message.success("资料提交成功！");
            this.$router.push("/dashboard");
          } else {
            this.$message.error(res.Message);
          }
        });
      }
    },
    checkPar(par, index) {
      let reg = /^[1-9]\d*$/;
      let f = {};
      f.flag = false;
      f.msg = "";
      if (par.Code == "") {
        return {
          flag: true,
          msg: "请输入验证码"
        };
      }
      if (this.autht != "") {
        par.id = this["s_" + this.autht].id;
      }
      let phoneflag = false;
      switch (par.phoneMode) {
        case "1":
          phoneflag = RegExp.phone.test(par.phone);
          break;
        case "2":
          phoneflag = RegExp.TWphone.test("886" + par.phone);
          break;
        case "3":
          phoneflag = RegExp.HKphone.test("82" + par.phone);
          break;
        case "4":
          phoneflag = RegExp.AUphone.test("61" + par.phone);
          break;
        default:
          phoneflag = false;
      }
      if (!phoneflag) {
        return {
          flag: true,
          msg: "请输入正确的手机号"
        };
      }
      if (index == 1) {
        // 微商代购
        if (par.operationCategory.length < 1) {
          f.flag = true;
          f.msg = this.errorMsg1();
          return f;
        } else if (par.userName == "" || !RegExp.chinese.test(par.userName)) {
          f.flag = true;
          f.msg = this.errorMsg4();
          return f;
        } else if (par.idCardNo == "" || !this.regIdCard(par.idCardNo)) {
          f.flag = true;
          f.msg = "请输入正确的身份证号码";
          return f;
        } else if (!par.idCardFrontImg) {
          f.flag = true;
          f.msg = this.errorMsg6();
          return f;
        } else if (!par.idCardBehindImg) {
          f.flag = true;
          f.msg = this.errorMsg7();
          return f;
        } else if (par.country == "中国" && par.area.length < 1) {
          f.flag = true;
          f.msg = this.errorMsg8();
          return f;
        } else if (par.weChat == "") {
          f.flag = true;
          f.msg = "请输入微信号";
          return f;
        }
        par.operationCategory = this.formartoperationCategory(
          par.operationCategory
        );
        par.interestCategory = this.formartinterestCategory(
          par.interestCategory
        );
        let cp = this.formartAddress(par.area);
        par.province = cp.province;
        par.city = cp.city;
        par.district = cp.district;
        par.areaCode = par.area[1];
        par.authType = 1;
        delete par.area;
        f.par = par;
        return f;
      } else if (index == 2) {
        // 网络店铺
        if (par.platform == "") {
          f.flag = true;
          f.msg = "请选择销售平台";
          return f;
        } else if (par.shopName == "") {
          f.flag = true;
          f.msg = "请输入店铺名称";
          return f;
        } else if (par.webLink == "" || !RegExp.Web.test(par.webLink)) {
          f.flag = true;
          f.msg = "请输入正确店铺网址";
          return f;
        } else if (par.operationCategory.length < 1) {
          f.flag = true;
          f.msg = "请选择经营类目";
          return f;
        } else if (par.userName == "" || !RegExp.chinese.test(par.userName)) {
          f.flag = true;
          f.msg = this.errorMsg4();
          return f;
        } else if (par.idCardNo == "" || !this.regIdCard(par.idCardNo)) {
          f.flag = true;
          f.msg = this.errorMsg5();
          return f;
        } else if (!par.idCardFrontImg) {
          f.flag = true;
          f.msg = this.errorMsg6();
          return f;
        } else if (!par.idCardBehindImg) {
          f.flag = true;
          f.msg = this.errorMsg7();
          return f;
        } else if (!par.shopImg) {
          f.flag = true;
          f.msg = "请上传店铺首页照片";
          return f;
        } else if (!par.saleCenterImg) {
          f.flag = true;
          f.msg = "请上传卖家中心照片";
          return f;
        } else if (!par.licensImg) {
          f.flag = true;
          f.msg = "请上传营业执照照片";
          return f;
        }
        par.operationCategory = this.formartoperationCategory(
          par.operationCategory
        );
        par.interestCategory = this.formartinterestCategory(
          par.interestCategory
        );
        par.authType = 2;
        f.par = par;
        return f;
      } else if (index == 3) {
        if (par.shopName == "") {
          f.flag = true;
          f.msg = "请输入店铺名称";
          return f;
        } else if (par.area.length < 1 || par.address == "") {
          f.flag = true;
          f.msg = this.errorMsg8();
          return f;
        } else if (par.shopCount == "" || !reg.test(par.shopCount)) {
          f.flag = true;
          f.msg = "请输入正确分店数量";
          return f;
        } else if (par.idCardNo == "" || !this.regIdCard(par.idCardNo)) {
          f.flag = true;
          f.msg = "请输入正确的身份证号码";
          return f;
        } else if (par.operationCategory.length < 1) {
          f.flag = true;
          f.msg = "请选择经营品类";
          return f;
        } else if (par.companyName == "") {
          f.flag = true;
          f.msg = "请输入企业名称";
          return f;
        } else if (!par.shopImg) {
          f.flag = true;
          f.msg = "请上传店铺照片";
          return f;
        } else if (!par.idCardFrontImg) {
          f.flag = true;
          f.msg = "请上传法人身份证正面照片";
          return f;
        } else if (!par.idCardBehindImg) {
          f.flag = true;
          f.msg = "请上传法人身份证反面照片";
          return f;
        } else if (!par.licensImg) {
          f.flag = true;
          f.msg = "请上传营业执照";
          return f;
        } else if (par.hasNetWorkShop == null) {
          f.flag = true;
          f.msg = "请选择是否拥有网络店铺";
          return f;
        } else if (par.hasNetWorkShop == true && par.platform == "") {
          f.flag = true;
          f.msg = "请选择销售平台";
          return f;
        } else if (par.hasNetWorkShop == true && par.webLink == "") {
          f.flag = true;
          f.msg = "请输入店铺网址";
          return f;
        }
        par.operationCategory = this.formartoperationCategory(
          par.operationCategory
        );
        par.interestCategory = this.formartinterestCategory(
          par.interestCategory
        );
        let cp = this.formartAddress(par.area);
        par.country = "中国";
        par.province = cp.province;
        par.city = cp.city;
        par.district = cp.district;
        par.areaCode = par.area[2];
        par.authType = 3;
        delete par.area;
        f.par = par;
        return f;
      } else if (index == 4) {
        if (par.shopName == "") {
          f.flag = true;
          f.msg = "请输入平台名称";
          return f;
        } else if (par.shopType == "") {
          f.flag = true;
          f.msg = "请选择平台属性";
          return f;
        } else if (par.webLink == "" || !RegExp.Web.test(par.webLink)) {
          f.flag = true;
          f.msg = "请输入正确店铺网址";
          return f;
        } else if (par.operationCategory.length < 1) {
          f.flag = true;
          f.msg = "请选择经营品类";
          return f;
        } else if (par.companyName == "") {
          f.flag = true;
          f.msg = "请输入企业名称";
          return f;
        } else if (par.idCardNo == "" || !this.regIdCard(par.idCardNo)) {
          f.flag = true;
          f.msg = "请输入正确的身份证号码";
          return f;
        } else if (!par.idCardFrontImg) {
          f.flag = true;
          f.msg = "请上传法人身份证正面照片";
          return f;
        } else if (!par.idCardBehindImg) {
          f.flag = true;
          f.msg = "请上传法人身份证反面照片";
          return f;
        } else if (!par.licensImg) {
          f.flag = true;
          f.msg = "请上传营业执照";
          return f;
        }
        par.operationCategory = this.formartoperationCategory(
          par.operationCategory
        );
        par.interestCategory = this.formartinterestCategory(
          par.interestCategory
        );
        par.authType = 4;
        f.par = par;
        return f;
      } else if (index == 6) {
        if (par.userName == "" || !RegExp.chinese.test(par.userName)) {
          f.flag = true;
          f.msg = this.errorMsg4();
          return f;
        } else if (par.idCardNo == "" || !this.regIdCard(par.idCardNo)) {
          f.flag = true;
          f.msg = "请输入正确的身份证号码";
          return f;
        } else if (!par.idCardFrontImg) {
          f.flag = true;
          f.msg = this.errorMsg6();
          return f;
        } else if (!par.idCardBehindImg) {
          f.flag = true;
          f.msg = this.errorMsg7();
          return f;
        } else if (par.area.length < 1 || par.address == "") {
          f.flag = true;
          f.msg = "请输入正确的收货地址";
          return f;
        }
        par.authType = 6;
        let cp = this.formartAddress(par.area);
        par.country = "中国";
        par.province = cp.province;
        par.city = cp.city;
        par.district = cp.district;
        par.areaCode = par.area[2];
        par.authType = 6;
        delete par.area;
        f.par = par;
        return f;
      }
    },
    formartstorePort(data) {
      let a = [];
      data.map(v => {
        a.push({
          storePort: v
        });
      });
      return a;
    },
    formartoperationCategory(data) {
      let a = [];
      data.map(v => {
        a.push({
          operationCategoryCode: v.split(",")[0],
          operationCategoryName: v.split(",")[1]
        });
      });
      return a;
    },
    formartinterestCategory(data) {
      let a = [];
      data.map(v => {
        a.push({
          interestCategoryCode: v.split(",")[0],
          interestCategoryName: v.split(",")[1]
        });
      });
      return a;
    },
    formartAddress(data) {
      let f = {};
      f.province = "";
      f.city = "";
      f.district = "";
      this.areaData.map(v => {
        if (v.Code == data[0]) {
          f.province = v.Name;
          v.Children.map(v1 => {
            if (v1.Code == data[1]) {
              f.city = v1.Name;
              v1.Children.map(v2 => {
                if (v2.Code == data[2]) {
                  f.district = v2.Name;
                }
              });
            }
          });
        }
      });
      return f;
    },
    errorMsg() {
      return "请输入正确的手机号码";
    },
    errorMsg1() {
      return "请选择经营品类";
    },
    errorMsg2() {
      return "请输入正确月销售额，1-9999万的正整数，单位：万";
    },
    errorMsg3() {
      return "请选择感兴趣类目";
    },
    errorMsg4() {
      return "请输入正确的店主姓名";
    },
    errorMsg5() {
      return "请输入正确的身份证号码";
    },
    errorMsg6() {
      return "请上传身份证正面照片";
    },
    errorMsg7() {
      return "请上传身份证反面照片";
    },
    errorMsg8() {
      return "请输入正确的店铺地址";
    },
    getResults() {
      API.authResult({
        token: localStorage.getItem("user")
          ? JSON.parse(localStorage.getItem("user")).token
          : null
      }).then(res => {
        this.loading = false;
        if (res.ResponseId == 0) {
          this.id = res.Data.Id;
          let obj = this.nameTo_(JSON.parse(JSON.stringify(res.Data)));
          console.log('obj: ', obj);

          this.autht = obj.authType;
          this.status = obj.status;
          let a = [];
          let b = [];
          obj.operationCategory.map(v => {
            a.push(v.OperationCategoryCode + "," + v.OperationCategoryName);
          });
          obj.operationCategory = a;
          obj.interestCategory.map(v => {
            b.push(v.InterestCategoryCode + "," + v.InterestCategoryName);
          });
          obj.interestCategory = b;
          if (obj.areaCode != "" && obj.authType != 1) {
            obj.area = this.getCityByCode(obj.areaCode);
          }
          if (obj.areaCode != "" && obj.authType == 1) {
            obj.area = this.getCityByCode2(obj.areaCode);
          }
          if (obj.status == 2) {
            this.FailReason = obj.failReason;
          }
          // plf
          if (obj.platform != "") {
            let f = false;
            this.plf.map(v => {
              if (v.name == obj.platform) {
                f = true;
              }
            });
            if (!f) {
              this.plf.push({
                name: obj.platform,
                value: obj.platform
              });
            }
          }
          if (obj.storePort.length > 0) {
            let b = [];
            obj.storePort.map(v => {
              let f = false;
              b.push(v.storePort);
              this.sp.map(item => {
                if (v.storePort == item.storePort) {
                  f = true;
                }
              });
              if (!f) {
                this.sp.push({
                  storePort: v.storePort
                });
              }
            });
            obj.storePort = b;
          }
          obj.hasNetWorkShop = res.Data.HasNetWorkShop;
          console.log(1111,obj.authType);
          console.log(2222,this["s_" + obj.authType]);
          
          this["s_" + obj.authType] = Object.assign(
            this["s_" + obj.authType],
            obj
          );
        } else if (res.ResponseId == -1) {
          this.$store.commit("loginStatus/LOGOUT");
          this.$message({
            message: "登录过期，请重新登录",
            type: "error",
            duration: 1000
          });
          this.$router.push({ path: "/login" });
        }else{
          this.$router.push({ path: "/authNew" });
        }
      });
    },
    getCityByCode(code) {
      let a = [];
      this.areaData.map(v => {
        v.Children.map(v1 => {
          v1.Children.map(v2 => {
            if (v2.Code == code) {
              a.push(v.Code);
              a.push(v1.Code);
              a.push(v2.Code);
            }
          });
        });
      });
      return a;
    },
    getCityByCode2(code) {
      let a = [];
      this.areaData.map(v => {
        v.Children.map(v1 => {
          if (v1.Code == code) {
            a.push(v.Code);
            a.push(v1.Code);
          }
        });
      });
      return a;
    },
    updateAuth() {
      this.status = -1;
    },
    toIndex() {
      // this.$store.commit("LOGOUT");
      this.$router.push({ path: "/" });
    },
    getAddress(cb) {
      API.getAddressData({
        token: localStorage.getItem("user")
          ? JSON.parse(localStorage.getItem("user")).token
          : null
      }).then(res => {
        res.Data.map(a => {
          a.Children.map(b => {
            b.Children.map(c => {
              delete c.Children;
            });
          });
        });
        let a = JSON.parse(JSON.stringify(res.Data));
        a.map(a => {
          a.Children.map(b => {
            delete b.Children;
          });
        });
        this.areaData = res.Data;
        this.areaData2 = a;
        cb();
      });
    },
    regIdCard(val) {
      if (!RegExp.IDcard.test(val)) return false;
      let idCardWi = new Array(
        7,
        9,
        10,
        5,
        8,
        4,
        2,
        1,
        6,
        3,
        7,
        9,
        10,
        5,
        8,
        4,
        2
      ); //将前17位加权因子保存在数组里
      let idCardY = new Array(1, 0, 10, 9, 8, 7, 6, 5, 4, 3, 2); //这是除以11后，可能产生的11位余数、验证码，也保存成数组
      let idCardWiSum = 0; //用来保存前17位各自乖以加权因子后的总和
      for (let i = 0; i < 17; i++) {
        idCardWiSum += val.substring(i, i + 1) * idCardWi[i];
      }
      let idCardMod = idCardWiSum % 11; //计算出校验码所在数组的位置
      let idCardLast = val.substring(17); //得到最后一位身份证号码
      //如果等于2，则说明校验码是10，身份证号码最后一位应该是X
      if (idCardMod == 2) {
        if (idCardLast == "X" || idCardLast == "x") {
          return true;
        } else {
          return false;
        }
      } else {
        //用计算出的验证码与最后一位身份证号码匹配，如果一致，说明通过，否则是无效的身份证号码
        if (idCardLast == idCardY[idCardMod]) {
          return true;
        } else {
          return false;
        }
      }
    },
    nameTo_(object) {
      for (let i in object) {
        let tl = i.charAt(0).toLowerCase() + i.slice(1);
        object[tl] = object[i];
        delete object[i];
      }
      return object;
    }
  },
  mounted() {
    this.uploadUrl =
      _CONFIG.fileUrl +
      "/f/upload/300002/?partnerId=20000&token=" +
      (localStorage.getItem("user") != null
        ? JSON.parse(localStorage.getItem("user")).token
        : "");
    this.getAddress(() => {
      this.getResults();
    });
  }
};
</script>
<style >
.input_link .input_selete .el-input__inner {
  width: 130px;
}
</style>
<style lang="less" scoped>
.input_link {
  width: 340px !important;
}
.mrl10 {
  margin-left: 10px;
}
.w240 {
  flex: 0 0 210px;
}
.w500 {
  width: 500px;
  display: flex;
}
.reUpload {
  position: absolute;
  color: #409eff;
  right: -70px;
}
.link_800_28 {
  line-height: 28px;
  width: 800px !important;
}
.text_left {
  width: 100% !important;
  text-align: left !important;
}
.auth_card {
  margin-bottom: 13px;
  color: #666;
  cursor: pointer;
  padding: 10px 0;
  font-size: 14px;
}
.auth_card_default {
  background-image: url(data:image/png;base64,);
  background-repeat: no-repeat;
  background-size: auto 40px;
  background-position: center center;
}
.auth_card_shing {
  background-image: url(data:image/png;base64,);
  background-repeat: no-repeat;
  background-size: auto 40px;
  background-position: center center;
}
.auth_card_passed {
  background-image: url(data:image/png;base64,);
  background-repeat: no-repeat;
  background-size: auto 40px;
  background-position: center center;
}
.auth_card_active {
  background-image: url(data:image/png;base64,);
  background-repeat: no-repeat;
  background-size: auto 40px;
  background-position: center center;
}
.color-red {
  color: #fe0000;
}
.topCon {
  padding-top: 20px;
  padding-bottom: 20px;
  background: #fff;
  border-bottom: 1px solid #ececec;
}
.tableCon .container {
  min-height: 600px;
  padding-top: 20px;
  padding-bottom: 20px;
}
.viewBtn {
  line-height: 32px;
  font-size: 20px;
  width: 80px;
  transition: all 0.2s ease-in;
  cursor: pointer;
  color: #333;
  &:first-child {
    margin-right: 40px;
  }
  &.active {
    border-bottom: 2px solid #fe0000;
  }
}
.view_split {
  display: inline-block;
  height: 20px;
  width: 2px;
  background: #ccc;
  position: relative;
  top: 6px;
  margin-right: 40px;
}
.table {
  padding-top: 20px;
  padding-bottom: 20px;
  font-size: 14px;
  color: #808080;
  .input_con {
    margin-top: 15px;
    display: flex;
    .input_label {
      width: 170px;
      text-align: right;
      line-height: 28px;
      margin-right: 10px;
    }
    .input_link,
    .input_address,
    .input_user,
    .input_job,
    .input_phone,
    .input_wechat {
      width: 240px;
    }
  }
}
.image_con {
  width: 238px;
  height: 148px;
  display: flex;
  position: relative;
  align-items: center;
  border: 1px solid #dcdfe6;
  text-align: center;
}
.image_con img {
  width: 100%;
  height: 100%;
}
.image_con p {
  width: 100%;
  flex: 1;
}
.btn_group {
  padding-left: 165px;
  margin-top: 30px;
}
.btn_sub {
  display: inline-block;
  width: 240px;
  line-height: 40px;
  background: #fe0000;
  color: #fff;
  font-size: 16px;
  text-align: center;
  font-weight: bold;
  border-radius: 2px;
}
.descAddress {
  margin-top: 10px;
}
.successInfo {
  padding-top: 60px;
  box-sizing: border-box;
  min-height: 600px;
  .icon_success {
    width: 70px;
    height: 70px;
    margin-right: 10px;
  }
  .info {
    .title {
      font-size: 18px;
      color: #333;
    }
    .color66 {
      color: #666;
      font-size: 12px;
    }
    .colorBlue {
      color: #24a7f7;
    }
  }
}
.input_con .color-red {
  line-height: 28px;
  margin-right: 5px;
  height: 28px;
}
</style>

<style>
.link_800 .el-checkbox {
  margin-left: 0 !important;
  margin-right: 30px;
}
</style>
